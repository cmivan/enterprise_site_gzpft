...
class Kindeditor {
	...
	{
		$mark = get_num($mark,0);
		if(is_num($w)==false&&is_num($h)==false){
			$w = 800;
			$h = 'auto';
		}elseif(is_num($w)==false){
			$w = 'auto';
		}elseif(is_num($h)==false){
			$h = 'auto';
		}
        ...
        if (empty($file) === FALSE) {
            ...
            //检查目录
            if(@is_dir($this->root_path) === false){ $this->alert("上传目录不存在。"); }
            //检查目录写权限
            if(@is_writable($this->root_path) === false){ $this->alert("上传目录没有写权限。"); }
            //检查是否已上传
            if(@is_uploaded_file($tmp_name) === false) { $this->alert("临时文件可能不是上传文件。"); }
            //检查文件大小
            if($file_size > $this->max_size) { $this->alert("上传文件大小超过限制。"); }
			//检查目录名
