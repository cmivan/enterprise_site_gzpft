...
class Products extends HT_Controller {
	...
	function __construct()
	{
		...
		if(!empty($pass)){
			$pass01 = $this->data['seo']['product.pass.add'];
			$pass02 = $this->data['seo']['product.pass.view'];
			if($pass==$pass01){
				$this->session->set_userdata("passAdd",'yes');
			}else{
		...
		$this->data['passAdd']= $this->session->userdata('passAdd');
		$this->data['passView'] = $this->session->userdata('passView');
		
		$this->data['passTitle'] = '管理';
	}
...
/* ********** 管理 *********** */
	function manage($type='')
	{
		...
		//分类检索
		$nottpe = $this->input->get('nottpe');
		$this->data['nottpe'] = '';
		if($nottpe=='no'){
			$this->data['nottpe'] = $nottpe;
			$this->db->where('i_type',NULL);
		}
