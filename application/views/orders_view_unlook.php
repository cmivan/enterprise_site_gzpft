...
<td align="center" class="changeObj"><div class="num"><?php echo getInput('num',$item->id,$item->num);?></div></td>
...
$(function(){
	$('.table-bordered').find('.num').find('input').change(function(){
		...
		var num   = liObj.find('.num').find('input').val();
		var val   = liObj.find('.num').find('input').attr('val');
		if( num!=''&&num==parseInt(num)&&num>0 ){
			...
			//累计所有价格
			_total = 0;
			liObj.parent().parent().find('.total').each(function(){
			...
			//还原数值
			liObj.find('.num').find('input').val(val);
			//alert('操作无效,订购数量必须是正整数!');
		}
	});
