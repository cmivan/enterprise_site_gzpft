<?php
class Products_real_Model extends CI_Model {
	...
	{
	    $this->db->select('*');
    	$this->db->from('products_real_type');
	...
	//返回最新的N条
	function top_products_real($num=0)
	{
	    $this->db->select('*');
	...
	function get_types_num()
	{
    	...
    	return $this->db->count_all_results('products_real_type');
	}
	...
	function hit($id=0)
	{
    	...
    	$this->db->where('id', $id);
    	return $this->db->update('products_real_type');
	}
	...
	{
    	$this->db->where('id', $id);
    	return $this->db->delete('products_real'); 
	}
	...
	function del_type($type_id)
	{
    	...
    	return $this->db->delete('products_real_type'); 
	}
